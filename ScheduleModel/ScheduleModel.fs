...
let earliestStart (alreadyScheduledEvents: ScheduledEvent list) (nextEvent:Event): (Time * Allocation) =
    ...
    let possibleStartTimes: Time list = 
        let earliestPossibleStartTimes = 
            alreadyScheduledEvents
            |> List.map (fun e -> e.finishTime) 
            |> List.distinct
            |> List.sort
        List.append [0] earliestPossibleStartTimes // Get all distinct end times + 0
    ...
    let tryTimeAndGetAllocation t: (Time * Allocation) option = 
        let overlappingEvents = // Get all overlapping events based on date range
            alreadyScheduledEvents
            |> List.filter (fun e -> e.startTime < (t + nextEvent.duration) && e.finishTime > t)

        let possibleAllocations = // Check each slot for an allocation
            [1..resourceAvailable nextEvent.location] 
            |> List.filter (fun a -> 
                overlappingEvents 
                |> List.filter (fun e -> e.event.location = nextEvent.location)
                |> List.forall (fun e -> e.allocated <> a))

        let isAgeGroupInOverlappingEvents =
            overlappingEvents
            |> List.exists (fun e -> e.event.ageGroup = nextEvent.ageGroup)

        if isAgeGroupInOverlappingEvents then None // Check next time if age group exists
        else
            if List.isEmpty possibleAllocations then None // No possible allocations, check next time
            else Some (t, List.min possibleAllocations) // Found a time with allocations, return time and minimum allocation for earliest time

    possibleStartTimes
    |> List.pick (fun startTime -> tryTimeAndGetAllocation startTime)
...
let scheduleNext (alreadyScheduled: ScheduledEvent list) (nextEvent: Event): ScheduledEvent list =
    let timeAndAllocation = earliestStart alreadyScheduled nextEvent
    List.append alreadyScheduled [{ 
        event = nextEvent; 
        startTime = fst timeAndAllocation; 
        finishTime = fst timeAndAllocation + nextEvent.duration; 
        allocated = snd timeAndAllocation 
    }]
...
let schedule (events: Event array) (order: int array) : ScheduledEvent list =
    ([], 0) 
    |> List.unfold (fun (scheduledEvents, index) -> 
        if index < events.Length then
            let nextEvent = events.[order.[index]]
            let nextScheduledEvents = scheduleNext scheduledEvents nextEvent
            Some (List.last nextScheduledEvents, (nextScheduledEvents, index + 1))
        else None)
...
let athleticsScheduleCost (events: Event array) = 
    let fitnessFunction (order: int array) : double = 
        ...
        (float (List.maxBy (fun e -> e.finishTime) (schedule events order)).finishTime) * -1.0
    fitnessFunction